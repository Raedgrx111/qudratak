...
cleanup() {
        if [ -n "${APP_PID:-}" ] && kill -0 "$APP_PID" >/dev/null 2>&1; then
                echo "Stopping Qudratak server (PID: $APP_PID)..."
...
log_step_start "Starting app server"
# قدراتك — الإنتاج أولًا: إن وُجد بناء standalone نشغّله (جاهز لـ 1200+ طالب)،
# وإلا نرجع مؤقتًا لوضع dev حتى يُبنى الإنتاج
if [ -f "$PROJECT_DIR/.next/standalone/server.js" ]; then
        echo "[QUDRATAK] Starting PRODUCTION server (standalone build)..."
        NODE_ENV=production bun .next/standalone/server.js &
else
        echo "[QUDRATAK] No production build found — falling back to dev mode"
        bun run dev &
fi
APP_PID=$!
log_step_end "Starting app server"

log_step_start "Waiting for app server"
wait_for_service "localhost" "3000" "Qudratak server"
log_step_end "Waiting for app server"
...
echo "Qudratak server is running in background (PID: $APP_PID)."
echo "Use 'kill $APP_PID' to stop it."
